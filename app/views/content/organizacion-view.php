...
<div class="modal fade" id="modalEdificios" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-dark text-white">
                <h5 class="modal-title" id="tituloEdificios">
                    <i class="fas fa-building me-2"></i> Gestión de Edificios
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" onclick="limpiarFormEdificio()"></button>
            </div>
                ...
                <div class="row">
                    <div class="col-md-4 border-end">
                        <form id="formEdificio">
                            <input type="hidden" id="edif_id_org"> <input type="hidden" id="edif_poligono"> <h6 class="fw-bold mb-3 text-primary">Agregar Nuevo Edificio</h6>
                            
                            <div class="mb-3">
                                <label class="form-label small fw-bold">Nombre del Edificio</label>
                                <input type="text" class="form-control" id="edif_nombre" placeholder="Ej: Edificio A - Rectoría">
                            </div>
                            
                            <div class="mb-3">
                                <label class="form-label small fw-bold">Número de Pisos</label>
                                <input type="number" class="form-control" id="edif_pisos" min="1" placeholder="Cantidad de plantas">
                            </div>

                            <div class="card border-dashed bg-light p-3 text-center mb-4">
                                <label class="small d-block mb-2 fw-bold text-muted">Ubicación en Mapa</label>
                                <button type="button" class="btn btn-info btn-sm rounded-pill shadow-sm" onclick="abrirMapaEdificio()">
                                    <i class="fas fa-draw-polygon me-1"></i> Dibujar Edificio
                                </button>
                                <div id="statusPoligonoEdif" class="mt-2 small text-danger fw-bold">
                                    <i class="fas fa-times-circle"></i> Perímetro no definido
                                </div>
                            </div>

                            <button type="button" class="btn btn-primary w-100 shadow" id="btnGuardarEdificio">
                                <i class="fas fa-plus-circle me-1"></i> Registrar Edificio
                            </button>
                        </form>
                    </div>

                    <div class="col-md-8">
                        <h6 class="fw-bold mb-3 text-secondary">Edificios en esta Organización</h6>
                        <div id="gridEdificios"></div>
                    </div>
                </div>
            </div>

            <div class="modal-footer bg-light">
                <button type="button" class="btn btn-secondary rounded-pill px-4" data-bs-dismiss="modal" onclick="limpiarFormEdificio()">Cerrar</button>
            </div>
        </div>
    </div>
</div>

<div class="modal fade" id="modalMapaEdificios" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-dark text-white">
                <h5 class="modal-title fw-bold"><i class="ri-map-2-line"></i> Dibujar Perímetro del Edificio</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" onclick="$('#modalEdificios').modal('show')"></button>
            </div>
            <div class="modal-body p-0">
                <div id="mapEdificios" style="height: 60vh; width: 100%;"></div>
            </div>
            <div class="modal-footer justify-content-start bg-light">
                <small class="text-muted"><i class="ri-information-line"></i> Dibuje el área exacta del edificio dentro del campus gris.</small>
            </div>
        </div>
    </div>
</div>
...
let mapEdif, drawnItemsEdif; // Mapa Edificios
let poligonoCampusActual = ""; 
...
// Función para abrir el modal en modo "CREAR"
    ...
    cargarOrganizaciones();
     ...
     // Esta función se activa desde el botón del Grid principal
    window.editarEdificios = function(id_org) {
        const grid = $("#grid").data("kendoGrid");
        const dataItem = grid.dataSource.get(id_org);
        ...
        if (dataItem) {
            ...
            $("#edif_pisos").val("");
            $("#edif_poligono").val("");
            $("#statusPoligonoEdif").html('<i class="fas fa-times-circle"></i> Perímetro no definido').addClass("text-danger").removeClass("text-success");
            
            // Asignar IDs y polígono de referencia del campus
            $("#edif_id_org").val(id_org);
            $("#tituloEdificios").html('<i class="fas fa-building me-2"></i> Edificios: ' + dataItem.nombre);
            poligonoCampusActual = dataItem.poligono; 

            // Cargar el listado de edificios en el grid del modal
            cargarGridEdificios(id_org);
            
            // ABRIR MODAL DE GESTIÓN (Donde se escribe el nombre y pisos)
            $("#modalEdificios").modal("show");
        }
    };

    // Abre el modal del mapa específico para edificios
    window.abrirMapaEdificio = function() {
        $("#modalEdificios").modal("hide");
        $("#modalMapaEdificios").modal("show");
    };

    // Acción de Guardar Edificio
    $("#btnGuardarEdificio").off('click').click(async function() {
        const id_org = $("#edif_id_org").val();
        const nombre = $("#edif_nombre").val().trim();
        const planta = $("#edif_pisos").val();
        const poligono = $("#edif_poligono").val();

        // VALIDACIÓN DE DATOS
        if (nombre === "" || !planta || planta < 1 || poligono === "") {
            alert("Error: Ingrese Nombre, Pisos y dibuje el área en el mapa.");
            return;
        }

        const formData = new FormData();
        formData.append("modulo_edificio", "registrar");
        ...
        try {
            const resp = await fetch('<?=APP_URL; ?>app/ajax/edificioAjax.php', { method: 'POST', body: formData });
            const res = await resp.json();
            if(res.icono === "success") {
                $("#edif_nombre").val("");
                $("#edif_pisos").val("");
                $("#edif_poligono").val("");
                $("#statusPoligonoEdif").html('<i class="fas fa-times-circle"></i> Perímetro no definido').addClass("text-danger");
                cargarGridEdificios(id_org);
                cargarOrganizaciones(); // Refrescar grid principal para ver conteo actualizado
            } else {
                alert(res.texto);
            }
    ...
    // ==========================================
    // 2. MAPA DE EDIFICIOS (LÓGICA INDEPENDIENTE)
    // ==========================================
    $('#modalMapaEdificios').on('shown.bs.modal', function () {
        if (!mapEdif) {
            mapEdif = L.map('mapEdificios').setView([19.7006, -101.1863], 16);
            L.tileLayer('https://{s}.google.com/vt/lyrs=y&x={x}&y={y}&z={z}', {
                maxZoom: 20, subdomains:['mt0','mt1','mt2','mt3']
            }).addTo(mapEdif);
            
            drawnItemsEdif = new L.FeatureGroup().addTo(mapEdif);
            const drawControlEdif = new L.Control.Draw({
                draw: { polygon: true, rectangle: true, polyline: false, circle: false, marker: false },
                edit: { featureGroup: drawnItemsEdif }
            });
            mapEdif.addControl(drawControlEdif);

            // EVENTO AL TERMINAR DIBUJO: Retorna siempre a modal de Edificios
            mapEdif.on(L.Draw.Event.CREATED, function (e) {
                const layer = e.layer;
                drawnItemsEdif.clearLayers();
                drawnItemsEdif.addLayer(layer);
                const coords = layer.getLatLngs()[0].map(c => ({ lat: c.lat, lng: c.lng }));
                
                $('#edif_poligono').val(JSON.stringify(coords));
                $('#statusPoligonoEdif').html('<i class="fas fa-check-circle text-success"></i> Área del edificio lista').removeClass('text-danger');
                
                $('#modalMapaEdificios').modal('hide');
                setTimeout(() => $('#modalEdificios').modal('show'), 400);
            });
        }

        // Dibujar el perímetro del campus como guía gris
        drawnItemsEdif.clearLayers();
        if (poligonoCampusActual) {
            try {
                const campusRef = L.polygon(JSON.parse(poligonoCampusActual), {
                    color: '#6c757d', fillOpacity: 0.1, dashArray: '5, 10', interactive: false
                }).addTo(mapEdif);
                mapEdif.fitBounds(campusRef.getBounds());
            } catch(e) { console.error("Error parseando campus"); }
        }
        mapEdif.invalidateSize();
    });

    // ==========================================
    // 3. MAPA DE ORGANIZACIONES (LÓGICA ORIGINAL)
    // ==========================================
    $('#modalMapa').on('shown.bs.modal', function () {
        if (!campusMap) {
            campusMap = L.map('map').setView([19.7006, -101.1863], 16);
            ...
            drawnItems = new L.FeatureGroup().addTo(campusMap);
            const drawControl = new L.Control.Draw({
                draw: { polygon: true, rectangle: true, polyline: false, circle: false, marker: false },
                edit: { featureGroup: drawnItems }
            });
            ...
            campusMap.on(L.Draw.Event.CREATED, function (e) {
                ...
                const coords = e.layer.getLatLngs()[0].map(c => ({ lat: c.lat, lng: c.lng }));
                
                $('#inputPoligono').val(JSON.stringify(coords));
                $('#statusPoligono').html('<i class="ri-checkbox-circle-line text-success"></i> Área delimitada').removeClass('text-danger');
...
async function cargarGridEdificios(id_org) {
    try {
        const response = await fetch('<?=APP_URL; ?>app/ajax/edificioAjax.php', {
            method: 'POST',
            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
            body: `modulo_edificio=listar&id=${id_org}`
        });
        const data = await response.json();
        
        if ($("#gridEdificios").data("kendoGrid")) {
            $("#gridEdificios").data("kendoGrid").destroy();
            $("#gridEdificios").empty();
        }

        $("#gridEdificios").kendoGrid({
            dataSource: { data: data, pageSize: 5 },
            height: 300,
            pageable: true,
            columns: [
                { field: "nombre", title: "Edificio" },
                { field: "planta", title: "Pisos", width: "80px", attributes: {class: "text-center"} },
                { 
                    title: " ", width: "70px", 
                    template: `<button class="btn btn-sm btn-outline-danger" onclick="eliminarEdificio(#: id #)"><i class="fas fa-trash"></i></button>` 
                }
            ]
        });
    } catch (e) { console.error("Error cargando grid edificios"); }
}

window.limpiarFormEdificio = function() {
    $("#edif_nombre").val(""); $("#edif_pisos").val(""); $("#edif_poligono").val("");
    $("#statusPoligonoEdif").html('<i class="fas fa-times-circle"></i> Perímetro no definido').addClass("text-danger");
};
</script>